...
ajuda.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
        document.getElementById("PageCadastroVeiculo").innerHTML = this.responseText;
    }
};
ajuda.open("GET", "cadastroVeiculo.html", true);
ajuda.send();
var ajuda = new XMLHttpRequest();
ajuda.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
        document.getElementById("pageVantagens").innerHTML = this.responseText;
    }
};
ajuda.open("GET", "clube-vantagens.html", true);
ajuda.send();
