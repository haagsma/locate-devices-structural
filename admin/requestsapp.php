...
function logar($conn){
    ...
    $pegaCelular = $email;
    $celular = "(".$pegaCelular[0].$pegaCelular[1].") ".$pegaCelular[2].$pegaCelular[3].$pegaCelular[4].$pegaCelular[5].$pegaCelular[6]."-".$pegaCelular[7].$pegaCelular[8].$pegaCelular[9].$pegaCelular[10];


    $select_ficha_motorista = "SELECT * FROM cadastro_motorista WHERE celular LIKE '$celular'";
...
function cadastrarMotorista($conn){
    ...
    if($senha != $senha_c){
        ...
        $inserir = "INSERT INTO cadastro_motorista (celular, nome_completo, cnh, email, senha) VALUES ( '$celular','$nome', '$cnh', '$email', '$senha_cript')";
...
function selectCampoMotorista($conn){
    ...
    $celular = "(".$email[0].$email[1].") ".$email[2].$email[3].$email[4].$email[5].$email[6]."-".$email[7].$email[8].$email[9].$email[10];
    $select = "SELECT $campo FROM cadastro_motorista WHERE celular LIKE '$email'";
...
function imgEndereco($conn){

    $id = $_POST['id'];
    //Inicio upload arquivo foto-icone
    $target_dir = "../assets/images/comprovante-endereco/";
    //$target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
    $target_file = $target_dir . basename("endereco-".$id.".png");
    $uploadOk = 1;
    $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
    // Check if image file is a actual image or fake image
    if(isset($_POST["submit"])) {
...
function enviarNF($conn){

    $id = $_POST['id'];
    //Inicio upload arquivo foto-icone
    $target_dir = "../assets/images/nf/";
    //$target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
    $target_file = $target_dir . basename("nf-".$id.".png");
    $uploadOk = 1;
    $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
    // Check if image file is a actual image or fake image
    if(isset($_POST["submit"])) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
            echo "Imagem Enviada!"; exit;
        } else {
            echo "Desculpe, o arquivo não foi carregado, tente novamente"; exit;
        }
    }
}
function enviarCTL($conn){

    $id = $_POST['id'];
    //Inicio upload arquivo foto-icone
    $target_dir = "../assets/images/ctl/";
    //$target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
    $target_file = $target_dir . basename("ctl-".$id.".png");
    $uploadOk = 1;
    $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
    // Check if image file is a actual image or fake image
    if(isset($_POST["submit"])) {
...
function imgCnh($conn){

    $id = $_POST['id'];
    //Inicio upload arquivo foto-icone
    $target_dir = "../assets/images/cnh/";
    //$target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
    $target_file = $target_dir . basename("cnh-".$id.".png");
    $uploadOk = 1;
    $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
    // Check if image file is a actual image or fake image
    if(isset($_POST["submit"])) {
        ...
        <div class="carga">
            <form id="aceitarCargaDisponivel<?= $col['id'] ?>">
                <div class="form-group">
                    ...
                    Tipo Carroceria: <?= $col['tipo_carreta'] ?><br>
                    Valor: R$ <?= $col['valor'] ?>
...
function telaCargaTransportadas($conn){
    $email = get('email');
    $query = "SELECT * FROM tbl_frete WHERE motorista = '$email' AND status = 'Concluido' ORDER BY id DESC";
    $result_query = $conn->query($query);
...
function telaCargaTransportadasCliente($conn){
    $email = get('email');
    $query = "SELECT * FROM tbl_frete WHERE empresa = '$email' AND status = 'Concluido' ORDER BY id DESC";
    $result_query = $conn->query($query);
    while($col = mysqli_fetch_assoc($result_query)){
        ?>
        <div class="carga">
            <form >
                <div class="form-group">
                    <div class="clearfix"></div>
                    <h3 class="title-carga">#<?= $col['id']." - ". $col['cidade_origem']." - ".$col['estado_origem']." a ".$col['cidade_destino']." - ".$col['estado_destino'] ?></h3>
                    Tipo veículo: <?= $col['tipo_veiculo'] ?><br>
                    Valor: R$ <?= $col['valor'] ?>
                    <div class="clearfix"></div>
                </div>
        ...
        <div class="carga">
            <form>
                <div class="form-group">
                    ...
                    Tipo veículo: <?= $col['tipo_veiculo'] ?><br>
                    Valor: R$ <?= $col['valor'] ?><br>
                    <button type="button" class="btn-atualizar finalizarCarga" value="<?= $col['id'] ?>" style="width: 90px">Finalizar Carga</button>
                    <button type="button" value="<?= $col['id'] ?>" class="btn-atualizar enviarNF nfcod<?= $col['id'] ?>" style="width: 90px">Enviar NF</button>
                    <button type="button" value="<?= $col['id'] ?>" class="btn-atualizar enviarCTL ctlcod<?= $col['id'] ?>" style="width: 90px">Enviar CTL</button>
                    <div class="clearfix"></div>
                </div>
            </form>
        </div>
    ...
    while($col = mysqli_fetch_assoc($result_query)){
        ...
        <div class="carga">
            <form>
                <div class="form-group">
                    <div class="clearfix"></div>
                    <h3 class="title-carga">#<?= $col['id']." - ".$col['titulo_aviso'] ?></h3>
                    <?= $col['mensagem'] ?><br>
                    <div class="clearfix"></div>
                </div>
            </form>
...
function telaVantagens($conn){
    $email = get('email');
    $query = "SELECT * FROM tbl_vantagens WHERE email = '$email' ORDER BY id DESC";
    $result_query = $conn->query($query);
    while($col = mysqli_fetch_assoc($result_query)){
        ?>
        <div class="carga">
            <form>
                <div class="form-group">
                    <div class="clearfix"></div>
                    <h3 class="title-carga">Seus Pontos</h3>
                    <p>Você tem <?= $col['pontos'] ?> pontos</p><br>
                    <div class="clearfix"></div>
                </div>
            </form>
        </div>
...
function cadastroVeiculo($conn){
    $tipo_veiculo = $rastreador = $placa_veiculo = $tipo_carreta = $marca = $modelo = $ano_fab = $ano_model = $placa_cavalo = $placa_carreta = "";
    ...
    $tipo_carreta = $_POST['tipo_carreta'];
    ...
    $inserir = "INSERT INTO tbl_cadastro_veiculo (tipo_carreta, renavam, proprietario_veiculo, placa, marca, modelo, ano_fab, ano_model, tipo_veiculo, placa_carreta, rastreador) VALUES ('$tipo_carreta','$renavam' ,'$proprietario', '$placa_veiculo', '$marca', '$modelo', '$ano_fab', '$ano_model', '$tipo_veiculo', '$placa_carreta', '$rastreador') ";
...
function selectCampoVeiculo($conn){
    class myObj {}
    $email = get('email');
    $query_placa = $conn->query("SELECT * FROM tbl_cadastro_veiculo WHERE proprietario_veiculo = '$email'");
    if($query_placa->num_rows > 0){
        $col = $query_placa->fetch_assoc();
            $myObj = new myObj();
            $myObj->placaVeiculo = $col['placa'];
            $myObj->renavam = $col['renavam'];
            $myObj->marcaVeiculo = $col['marca'];
            $myObj->modeloVeiculo = $col['modelo'];
            $myObj->anoFab = $col['ano_fab'];
            $myObj->anoMod = $col['ano_model'];
            $myObj->placaCarreta = $col['placa_carreta'];
            $myObj->tipoCarreta = $col['tipo_carreta'];
            $myObj->tipoVeiculo = $col['tipo_veiculo'];
            $myObj->rastreador = $col['rastreador'];

        $myJSON = json_encode($myObj);
        echo $myJSON;
    }
}
function removerVeiculo($conn){
    $email = get('email');
    $conn->query("DELETE FROM tbl_cadastro_veiculo WHERE proprietario_veiculo = '$email'");
}
function finalizarCarga($conn){
    $id = post('id');
    $mensagem = "Motorista informa que a carga $id foi finalizada";
    $data = date("Y-m-d");
    $query = "INSERT INTO `tbl_aviso` (`data`, `remetente`, `titulo_aviso`, `mensagem`) VALUES ('$data', '99 Cargas', 'Carga Finalizada', '$mensagem')";
    $conn->query($query);
    echo "Mensagem enviada com sucesso!";
}
